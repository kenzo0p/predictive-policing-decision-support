...
REQUIRED_COLUMNS = ["state", "district", "year", "total_crimes"]
...
def _infer_column_map(columns: List[str], user_map: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    if user_map:
        ...
        return {std: user_map[std] for std in REQUIRED_COLUMNS if std in user_map}
...
def _compute_total_crimes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    candidates = [c for c in numeric_cols if c not in {"year"}]
    if not candidates:
